...
s=1;
k5=1;
filename = '/home/gxa131/mmm/data/CCF_Atrial_Fibrillation/Afib-ablation-data-20190111/Final_Labels.xlsx';
projectdir='/home/gxa131/mmm/data/CCF_Atrial_Fibrillation/Afib-ablation-data-20190111/CT-segmentations';
ds = spreadsheetDatastore('/home/gxa131/mmm/data/CCF_Atrial_Fibrillation/Afib-ablation-data-20190111/Final_Labels.xlsx');
p = read(ds);
columnC = p.Recurrence_3m_1y_;
columnA=p.ID;
columnB = p.LumenModel;
columnD = p.PVModel;
columnH = p.TissueModel;
for i2=1:2
...
co = sum(sum(sum(X)));
M=Y(X>0)
[fds ics averFD averIC]= fdvolfft(M);   %3d
...
%%%%
%}
aaa(1,3)=median(fds_main(~isnan(fds_main)), 'all');   %3D_shape_median
%aaa(1,13)=median(ff0, 'all');       %Median_FD2_shape
%aaa(1,14)=max(max(ff0));            %Maximum_FD2_shape
aaa(1,4)=max(max(fds));               %Maximum_FD3_shape
aaa(1,5)=std(std(fds));            % 3D shape_std
%aaa(1,17)=std(ff0);
aaa(1,6)=skewness(skewness(fds));      %3D_shape_skewness
%{
...
FF(i2,1:7)=[features Labels];
clear J1 F FD ff0 FDavg aIC co D FDsd aFD
end
save F_tissue_features FF
